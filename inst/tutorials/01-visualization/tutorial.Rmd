...
output: 
  learnr::tutorial:
    progressive: true
    allow_skip: true
author: Jessica Edwards, Connor Rust, Liam Rust
...
```{r point-I-1, exercise=TRUE, exercise.lines=10}
qscores %>%
...
For data set `sps`, make a scatterplot with `age` on the x axis and `health_exp_3m` on the y axis.
...
Nice job! As you can see, our plot has many overlapping points making it hard to tell what's going on. Let's change the opacity by setting `alpha argument` to 0.1.
```{r point-I-3, exercise=TRUE, exercise.lines=8}
...
```{r point-I-4, exercise=TRUE, exercise.lines=10}
...
Now add the label "Medical Expenses over 3 Months" to the y axis and the label "Age" to the x axis. Next, add the title "Age and Medical Expenses in Mexico".

```{r point-I-5, exercise=TRUE, exercise.lines=10}



```

```{r point-I-5-hint, eval=FALSE}
Use labs()
...
Use the `annotate()` function to draw a yellow rectangle with an alpha of .1 around points that represent classes with over 400 students.

```{r point-II-4, exercise=TRUE, exercise.lines=8}


```

```{r point-II-4-hint, eval=FALSE}
 annotate(geom = "...", xmin = ..., xmax = ..., ymin = ..., ymax = ..., fill = ..., alpha = ...)
...
Awesome! Use `annotate()` again to label the rectangle with "high enrollment classes"

```{r point-II-5, exercise=TRUE, exercise.lines=8}


```

```{r point-II-5-hint, eval=FALSE}
...
Let's make a scatterplot using `iris`. Hit the 'Run Code' button.
...
Nice! Now let's make a scatterplot with x = Petal.Width, and y = Sepal.Length. While we're at it, let's also jitter the plot to reduce overplotting.

```{r gp-1-4-hint, eval=FALSE}
To make a jittered scatterplot, use geom_jitter().
```

```{r gp-1-4, exercise=TRUE, exercise.lines = 5}
...
Let's make a scatterplot using the `diamonds` data set, with `price` on the x-axis, and `carat` on the y-axis.

```{r gp-2-1-hint, eval=FALSE}
To learn about how to make a scatterplot, refer to textbook section 1.3.
```
...
Welcome to geom_jitter() I! Let's start by looking at the data set `trains`, of which I have made a scatterplot below. Press "Run Code":

```{r jit-I-1, exercise=TRUE, exercise.lines=6}
trains %>% 
  ggplot(aes(x = att_start, y = att_end)) +
    geom_point()
...
```{r jit-I-2, exercise=TRUE, exercise.lines=6}
trains %>% 
  ggplot(aes(x = att_start, y = att_end)) +
    geom_...()
...
As you can see, this changes things. The data on our graph was affected by overplotting, which is where points are plotted on top of eachther. Jitter shifted them slightly from under each other. We can adjust the amount the points are jittered by setting the width. Set the width to .2:
...
Great! Now add the title "Attitudes"

```{r jit-I-5, exercise=TRUE, exercise.lines=6}


```

```{r jit-I-5-hint, eval=FALSE}
Use labs()
```
## geom_jitter() II
...
Welcome to geom_jitter()II! Start by using data set `nhanes` to make a scatterplot jittered by width .5, with `education` on the x axis, and `bmi` on the y axis.
...
Let's make a jittered scatterplot using `geom_jitter`, and the `diamonds` tibble, which contains information about cut diamonds. Hit 'Run Code' to continue.
...
Well done. Now we can add some labels with labs() to finish off our plot. Title it "Price and Size in Diamonds", call our x-axis "Size (Carats)", and our y-axis "Price (US Dollars)"

```{r gj-1-4, exercise=TRUE, exercise.lines = 5}
...
Make a jittered scatterplot using `geom_jitter()` and the `diamonds` tibble. Let's put `carat` on the y-axis, and `price` on the x-axis.
...
```{r line-I-1, exercise=TRUE, exercise.lines=6}
economics %>%
...
###
Welcome to geom_line() II! Let's revisit the data set `economics` from geom_line() I. Change the scale on the y axis of the graph below to log base 10 and the color of the line to "blue".
...
```{r line-II-2-hint, eval=FALSE}
 ...
 Use the scales library in set labels= inside of the scale function
```
...
population %>% filter(country == "Canada") %>% ggplot(aes(x = year, y = population)) + 
  geom_line()
```
...
```{r gl-1-2, exercise=TRUE, exercise.lines = 5}
...
```{r gl-1-3, exercise=TRUE, exercise.lines = 5}
...
```{r}
population %>% filter(country == "Germany" | country == "France" | country == "Italy") %>% ggplot(aes(x = year, y = population, linetype = country, color = country)) + 
  geom_line() + 
...
Create a `line()` graph using the `economics` tibble, with x = `date`, and y = `unemploy`.
...
Nice! Now, let's add some labels. Title our plot "Unemployment by Year", our x-axis "Year", and our y-axis "Percentage of Population Unemployed".
...
```{r hist-I-1, exercise=TRUE, exercise.lines=6}
qscores %>%
  ggplot(aes(x = hours)) +
    geom_histogram()
```
...
Change the x axis variable to `rating`
```{r hist-I-2, exercise=TRUE, exercise.lines=7}


```


```{r hist-I-2-hint, eval=FALSE}
qscores %>%
  ggplot(aes(x = ...)) +
    geom_histogram()
```
...
Nice! Now change the colors of the bins to "red4" (the r code for crimson). 
```{r hist-I-3, exercise=TRUE, exercise.lines=7}


```


```{r hist-I-3-hint, eval=FALSE}
Set the fill argument in geom_histogram() to "red4"
```

###

Great! Now add a "white" outline to the bins.

```{r hist-I-4, exercise=TRUE, exercise.lines=7}


```

```{r hist-I-4-hint, eval=FALSE}
Set the color argument in geom_histogram() to "white"
```
###

Now add the title "Course Rating Distribution" and labels of your choice to both axes

```{r hist-I-5, exercise=TRUE, exercise.lines=7}


```

```{r hist-I-5-hint, eval=FALSE}
Use labs()
```
...
Let's keep working with our plot from geom_histogram() I, which you can see below. See the message R gave us about bin widths? Change the bin width by setting the number of bins to 20.
```{r}
qscores %>%
  ggplot(aes(x = rating)) +
...
Let's make a histogram using geom_histogram() and the `mgp` tibble, which contains data on the fuel efficiency of different types of cars. Press 'Run Code' to continue.
...
Wonderful. Now, let's add some labels using `labs()`. Let's title our plot "Highway Fuel Economy Distribution", our x-axis "Miles Per Gallon (Highway)", and our y-axis "Number of Car Models"

```{r gh-1-4, exercise=TRUE, exercise.lines = 5}
...
```{r gh-1-5, exercise=TRUE, exercise.lines = 5}
...
```{r box-I-1, exercise=TRUE, exercise.lines=10}
trains %>%
  ggplot(aes(x = party, y = att_end)) +
    geom_boxplot()
```

###  

Change the code for the previous graph so that `party` is on the x axis and `att_start` is on the y axis
```{r box-I-2, exercise=TRUE, exercise.lines = 10}
trains %>%
  ggplot(aes(x = ..., y = ...)) +
    geom_boxplot()
...
For dataset `governors`, create a boxplot where `party` is on the x axis and `win_margin` is on the y axis

```{r box-I-3, exercise=TRUE, eval=FALSE, exercise.lines=10}

```

```{r box-I-3-hint,eval=FALSE}
... %>%
  ggplot(aes(x = ..., y = ...)) +
    geom_boxplot()
```
...
Nice! Now add a title and labels on both axes

```{r box-I-5, exercise=TRUE, eval=FALSE, exercise.lines=10}

```


```{r add-label-hint,eval=FALSE}
Use labs() 
```
...
Let's make a `boxplot()` using the `mpg` tibble, which shows the fuel efficiency for different types of cars. Hit Run Code.

```{r gb-1-1, exercise=TRUE, exercise.lines = 5}
...
Now make a `boxplot()` with x = `class`, and y = `hwy`. Feel free to use copy and paste from the previous question.
...
```{r}
mpg %>% ggplot(aes(x = manufacturer, y = cty)) +
...
Make a `boxplot()` using the `diamonds` tibble with x = `cut`, and y = `price`. Which 
...
```{r}
diamonds %>% ggplot(aes(x = price, y = carat, group = cut_interval(price, n = 4))) + 
...
Nice. Now, let's try changing the `color`, or outline, of our 'violins' to "red".
...
Great. Now, let's add some labels with `labs()`. Let's call our plot "Price and Quality in Diamonds", our x-axis "Diamond Quality", and our y-axis "Price (US Dollars)."

```{r gv-1-3, exercise=TRUE, exercise.lines = 5}

```

###

Cool. Now, let's use `draw_quantiles` to draw percentile lines on our violins, at 0.25, 0.5, and 0.75.
...
```{r}
diamonds %>% ggplot(aes(x = cut, y = carat), color = "blue") + 
  geom_violin() +
  labs(title = "Size and Quality in Diamonds",
       x = "Quality",
       y = "Size (Carats)")
...
Well done. Now, let's add some percentile lines with `draw_quantiles` at 0.33 and 0.66.Let's also set `trim` to FALSE, to add tails to the violins.
...
Welcome to geom_violin() I. We are first going to be looking at the data set `diamonds`. Press "run code":

```{r viol-I-1, exercise=TRUE, exercise.lines=8}
diamonds %>% 
  ggplot(aes(x = clarity, y = price)) +
    geom_violin()
```
...
Try switching `price` for `carat` on the y axis
```{r viol-I-2, exercise=TRUE, exercise.lines=8}
diamonds %>% 
  ggplot(aes(x = clarity, y = ...)) +
    geom_violin()
```
...
Great! Now color the plots "red3" using the `fill` argument in `geom_violin()`

```{r viol-I-5, exercise=TRUE, exercise.lines=8}
...
```{r col-I-5, exercise=TRUE, exercise.lines=7}
starwars %>%
  filter(species == "Droid") %>%
...
```{r col-II-1, exercise=TRUE, exercise.lines=7}
starwars %>% 
...
qscores %>% 
  filter(department == "ENGLISH") %>%
    ggplot(aes(x = course_number, y = hours)) +
      geom_col() +
      theme(axis.text.x = element_text(size = 5)) + 
      labs(title = "Hours of Work in Harvard English Classes")
```

```{r col-II-5, exercise=TRUE, exercise.lines=7}
qscores %>% 
  filter(department == "ENGLISH") %>%
```
...
qscores %>% filter(department == "MUSIC") %>% ggplot(aes(x = name, y = rating)) +
  geom_col()
```
...
qscores %>% filter(department == "MUSIC") %>% ggplot(aes(x = ..., y = ...)) +
  geom_col()
```
...
```{r}
qscores %>% filter(department == "NEURO") %>% ggplot(aes(x = course_name, y = hours)) +
  geom_col() + 
...
```{r}
mpg %>% filter(manufacturer == "audi") %>% ggplot(aes(x = displ, y = hwy, fill = drv)) + 
  geom_col() +
  theme_bw() + 
  scale_fill_discrete(name = "Drive Mode", labels = c("Front Only", "Four Wheel")) + 
...
```{r smooth-I-2, exercise=TRUE, exercise.lines=7}
nhanes %>%
  ggplot(aes(x = weight, y = height)) + 
    geom_point() + 
    geom_...()
...
```{r}
iris %>% ggplot(aes(x = Sepal.Width, y = Sepal.Length, linetype = Species)) + 
  geom_smooth(se = FALSE, color = "purple") + 
  labs(title = "Sepal Width and Length in Iris Species",
       x = "Sepal Width",
       y = "Sepal Height") +
  scale_linetype_discrete(name = "Iris Species:", labels = c("Setosa Iris:", "Blue Flag Iris:", "Virginia Iris:")) +
...
```{r bin-I-2, exercise=TRUE, exercise.lines=6}
diamonds %>%
  ggplot(aes(x = ..., y = ...)) +
    geom_bin2d()
```
...
```{r}
qscores %>% ggplot(aes(x = enrollment, y = rating)) + 
  geom_bin2d() +
...
```{r gd2d-1-5, exercise=TRUE, exercise.lines = 5}
...
```{r data-III-6, exercise=TRUE, exercise.lines=6}
qscores <- qscores %>%
...
```{r data-III-7, exercise=TRUE, exercise.lines=6}
qscores <- qscores %>%
...
airquality %>% select(Month, Day, Temp) 
```
...
```{r da-1-5, exercise=TRUE, exercise.lines = 5}
airquality %>% select(Month, Day, Temp) %>%
```
...
```{r}
mpg %>% filter(class != "compact") %>% select(displ, manufacturer) %>% group_by(manufacturer) %>% summarize(displMedian = median(displ)) %>% arrange(desc(displMedian))
```
...
```{r}
qscores %>% select(rating, department, enrollment) %>% filter(enrollment > 25) %>% group_by(department) %>% summarise(medianRating = median(rating), deviation = mad(rating))
```
